...
import numpy as np
class Classification(nn.Module):  #可能用不上
...
class UnsupervisedLoss(object):
    ...
    def __init__(self, graph, train_nodes, device):
        ...
        for node in nodes:
            ...
            for i in range(self.N_WALK_LEN):
                ...
                for outer in frontier:
                    current |= self.graph.neighbor(0,int(outer))
    ...
    def _run_random_walks(self, nodes):
        for node in nodes:
            if len(self.graph.neighbor(0,int(node))) == 0:
                continue
            cur_pairs = []
            for i in range(self.N_WALKS):
                curr_node = node
                for j in range(self.WALK_LEN):
                    neighs = self.graph.neighbor(0, int(curr_node))
...
class GraphSage(nn.Module):
    ...
    def __init__(self, num_layers, input_size, out_size, graph, device, gcn=False, agg_func='MEAN'):
        ...
        self.graph = graph
        self.raw_features = self.get_raw_feature(offset=0)
    ...
    def get_raw_feature(self):
        raw_feature = np.eye(self.feature_dim)
        return raw_feature
    ...
    def _get_unique_neighs_list(self, nodes, num_sample=10):
        _set = set
        to_neighs = [self.graph.neighbor(0, int(node)) for node in nodes] # index maybe wrong
